    ...
    <>
      <div className="max-w-full bg-white flex flex-col rounded overflow-hidden shadow-lg">
        <div className="flex flex-row items-baseline flex-nowrap bg-gray-100 p-2">
          <img className="h-10 w-10 rounded-full mr-4" alt="Logo" />
          <h1 className="ml-2 uppercase font-bold">Starting Date</h1>
          <p className="ml-2 font-base text-gray-500">{game.staringDate}</p>
        </div>
        ...
        <div className="mt-2 flex sm:flex-row mx-6 sm:justify-between flex-wrap ">
          ...
          <div className="flex flex-col p-2">
            <p className="font-bold">Avaiable People</p>
            <p>{game.maxPlayers - game.peopleBooked.length}</p>
          </div>
          <div className="flex flex-col p-2">
            <p className="font-bold">Staring Time</p>
            <p className="font-base">
              {moment(game.startTime, "HH:mm").format("hh:mm A")}
            </p>
          </div>
          <div className="flex flex-col flex-wrap p-2">
            <p className="font-bold">End Time</p>
            <p className="font-base">
              {moment(game.endTime, "HH:mm").format("hh:mm A")}
            </p>
          </div>
        </div>
          ...
          <div className="md:border-l-2 mx-6 md:border-dotted flex flex-row py-4 mr-6 flex-wrap">
            ...
            <button
              ...
              onClick={() => {
                ...
                localStorage.setItem("idTrip", game._id);
              }}
            >
